...
public class AimingCalculator {
    ...
    public static ShamperAndRotationState calculateStill() {
        double robotAngle;
        double shamperAngle;
        ...
        double fullDistanceToSpeaker = new Pose2d(114, 0, new Rotation2d(0)).getTranslation()
                .getDistance((RobotState.getInstance().isRedAlliance()) ? Constants.FieldAndRobot.RED_SPEAKER_LOCATION
                        : Constants.FieldAndRobot.BLUE_SPEAKER_LOCATION);
        ...
        double horizontalVelocityNeeded = (fullDistanceToSpeaker / timeUntilTrajectoryTargetHeight);

        shamperAngle = Math.atan(verticalVelocityNeeded / horizontalVelocityNeeded);
        
        robotAngle = Math.toDegrees(angleToSpeaker) + Constants.Drivetrain.ROBOT_AIMING_ROTATION_OFFSET_IN_DEGREES;

        return new ShamperAndRotationState(shamperAngle, 0);
    }
}
